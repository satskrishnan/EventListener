...
import com.digital.core.banking.repository.OrderRepository;
import com.digital.core.banking.repository.PetRepository;
...
public class StoreApiDelegateImpl implements StoreApiDelegate {
	...
	public ResponseEntity<Order> getOrderById(Long orderId) {
		return orderRepository.findById(orderId).map(ResponseEntity::ok)
	...
	private static Order createOrder(long id, long petId, Order.StatusEnum status) {
		return new Order().id(id).petId(petId).quantity(2).shipDate(new Date()).status(status);
	}
}
